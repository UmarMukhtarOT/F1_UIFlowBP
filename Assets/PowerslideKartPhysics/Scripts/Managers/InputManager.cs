        ...
        void Update() {
            ...
            try {
                ...
                if (enableMobileInput) {
                    ...
                    // Usman Nawaz :: Logic Start

                    if (DoubleTapSteerCheck.leftSteer && DoubleTapSteerCheck.rightSteer)
                    {
                        MobileInput.steerInput = 0;
                        MobileInput.brakeInput = 1;
//#if UNITY_EDITOR 
//                        if (Input.GetKeyDown(KeyCode.C))
//                        {
//                            ReferenceManager.instance.ftue_manager.DeActiveBothTutorialHandForSpeedDown();

//                        }
//#endif
//#if UNITY_IOS
                        if (ReferenceManager.instance.trackManager.CurrentTrack.isTutorialTrack  && !ReferenceManager.instance.ftue_manager.singleHandTutorial)
                        {
                            ReferenceManager.instance.ftue_manager.DeActiveBothTutorialHandForSpeedDown();
...
//#endif
                    ...
                    else if (DoubleTapSteerCheck.leftSteer)
                    {
                        MobileInput.steerInput = -1;
                        if (ReferenceManager.instance.trackManager.CurrentTrack.isTutorialTrack && ReferenceManager.instance.ftue_manager.singleHandTutorial)
                        {
                            ReferenceManager.instance.ftue_manager.DeActiveTutorialLeftSteerHand();
                        }
                    }
                    else if (DoubleTapSteerCheck.rightSteer)
                    {
                        MobileInput.steerInput = 1;
                        if (ReferenceManager.instance.trackManager.CurrentTrack.isTutorialTrack && ReferenceManager.instance.ftue_manager.singleHandTutorial)
                        {
                            ReferenceManager.instance.ftue_manager.DeActiveTutorialRightSteerHand();
                        }
                    }

                    // Usman Nawaz :: Logic End
        ...
        public void PressItemMobile() {
            ReferenceManager.instance.collectableSpinnerHandler.OnSpinnerItemUsedEvent.Invoke();

            //CollectableSpinnerHandler.OnSpinnerItemUsedEvent;
