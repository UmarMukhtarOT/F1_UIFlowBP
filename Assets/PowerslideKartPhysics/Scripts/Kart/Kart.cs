...
namespace PowerslideKartPhysics
{
    ...
    public class Kart : MonoBehaviour
    {
        ...
        private void Update() {
            ...
            if (boostType == KartBoostType.DriftManual) // Drift manual boost input
            {
                Debug.Log("Boost Heree....");
                ...
                if (canBoost && boostButtonDown)
                {
                    ...
                    if (1 >= driftManualBoostLimit)
                    {
                        Debug.Log("Boost success");
                        //boostCount++;
                        boostCount = 1;
                        AddBoost(4 * boostCount * boostPower, boostCount);
        ...
        public void AddBoost(float boostToAdd) {
            if (canBoost) {
                // Original without Usman Nawaz change
                ...
                if (boostType == KartBoostType.DriftAuto || boostType == KartBoostType.DriftManual || boostType == KartBoostType.Manual)
                {
                    boostReserve = Mathf.Min(boostReserve + boostToAdd, boostReserveLimit);
                }
        ...
        public void AddBoost(float boostToAdd, float pushForce) {
            ...
            {
                AddBoost(boostToAdd);
                //ReferenceManager.instance.gameManager.camera.GetComponent<CameraFilterPack_Blur_Radial_Fast>().enabled = true;
                rb.AddForce(forwardDir * pushForce, ForceMode.VelocityChange);
            }
        ...
        void CancelDriftBoost(bool failed) {
            ...
            boostFailed = failed;
            boostCount = 0;
            //ReferenceManager.instance.gameManager.camera.GetComponent<CameraFilterPack_Blur_Radial_Fast>().enabled = false;
        ...
        public bool IsBoostReady() {
            //Debug.Log("is boost ready is true, Boost type is: "+ boostType);
